...
impl DictDB {
    ...
    pub fn new() -> Vec<DictDB>{
        ...
        let mut string_bg_en = String::new();
        {
            ...
            let mut file_en_bg = File::open("/usr/local/share/bedic/en_bg-utf8.dat").expect("Unable to open file en_bg-utf8.dat");
            let mut file_bg_en = File::open("/usr/local/share/bedic/bg_en-utf8.dat").expect("Unable to open file bg_en-utf8.dat");

            file_en_bg.read_to_string(&mut string_en_bg).expect("Unable to read file en_bg-utf8.dat");
            file_bg_en.read_to_string(&mut string_bg_en).expect("Unable to read file bg_en-utf8.dat");
        }
        ...
        info!("This database contains {} elements", concatenated_dictionaries.len());
    ...
    pub fn search(searched_txt: &str, data: &Vec<DictDB>) -> Result<Vec<DictDB>, Vec<DictDB>> {
        ...
        let search_struct = DictDB { word: searched_txt.to_string().to_uppercase(), translation: "_".to_string() };
...
trait VecDictDB {
    ...
    fn my_binary_search_by<'a, F>(&'a self, f: F) -> Result<usize, usize>
        where F: FnMut(&'a DictDB) -> Ordering;
}
